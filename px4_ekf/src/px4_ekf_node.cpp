...
void extended_kalman_filter_old(sensor_msgs::NavSatFix gps_data, geometry_msgs::Twist gps_pos, geometry_msgs::Twist gps_stm_pos, 
  sensor_msgs::Imu imu_data, geometry_msgs::Twist bicycle_data, geometry_msgs::Twist* state);
void WsgToTWD(double raw_lat, double raw_lon, float* output);
...
void stmGPSCallback(const geometry_msgs::Twist::ConstPtr& msg){
  ...
  if(!(gps_stm_pos.linear.x == x_old && gps_stm_pos.linear.y == y_old)){
    UPDATE_STM = 1;
  }
  else{
    UPDATE_STM = 0;
  }
  if(std::isfinite(gps_stm_pos.linear.x)&&gps_stm_pos.linear.x<1000&&gps_stm_pos.linear.x>-1000){
    x_old = gps_stm_pos.linear.x;
  }
  else{gps_stm_pos.linear.x= x_old;}
  if(std::isfinite(gps_stm_pos.linear.y)&&gps_stm_pos.linear.y<1000&&gps_stm_pos.linear.y>-1000){
    y_old = gps_stm_pos.linear.y;
  }
...
void extended_kalman_filter(sensor_msgs::NavSatFix gps_data, geometry_msgs::Twist gps_pos, geometry_msgs::Twist gps_stm_pos, 
  ...
  #ifndef USE_STM_DATA
  float theta = roll;
  float theta_dot = imu_data.angular_velocity.x;
  ...
  // Mt << 0.01,0,
  //       0,0.01;
  float alpha1 = 0.1; //0.001; //0.03
  float alpha2 = 0.1; //0.0001;
  ...
  static Eigen::Matrix4f covariance_est = (Eigen::Matrix4f() << 0.7, 0, 0, 0, 
                                                                0, 0.7, 0, 0, 
                                                                0, 0, 0.3, 0,
                                                                0, 0, 0, 0.1).finished();
