...
#define INCREASERATE    0.5f
#define DECREASERATE    0.5f
#define SPAWNVAL        8
    ...
    if(input_cloud->size() == 0){
        ...
        for (it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
            ...
            for (pit = it->indices.begin(); pit != it->indices.end(); pit++) {
                ...
                #endif
                // cout<<input_cloud->points[*pit].x<<" "<<input_cloud->points[*pit].y<<" "<<input_cloud->points[*pit].z<<std::endl;
                numPts++;
            }
