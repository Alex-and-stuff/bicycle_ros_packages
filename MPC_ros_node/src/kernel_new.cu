...
#define T_HRZ       4.0f			// Prediction time horizon 3.2
...
#define V_RAND_SCALAR 2.0f
#define W_RAND_SCALAR 1.0f
#define W_DIFF      0.04f  //0.4

#define OFF_ROAD_COST	700.0f		// Penalty for leaving the road
...
__global__ void genControlNew(uint64_t seed, Control* rand, Control* u, float v_scalar, float w_scalar) {
	/*  Generate V from nominal control U and the generated pertubations E (V = U + E)
		mean 0.0 and standard deviation 1.0, scalar: stdev scalar*/

	int idx = threadIdx.x + blockIdx.x * blockDim.x;
	rand[idx].vd = getRandom(seed, idx, 0) * v_scalar + u[threadIdx.x].vd;
	rand[idx].wd = getRandom(seed, idx, 10) * w_scalar + u[threadIdx.x].wd;
	clampingFcn(&rand[idx],1);
}
...
__global__ void costFcn(float* cost_list, State* state_list,  
  ...
  // state_cost += distance / (ROAD_WIDTH / 2) * TRACK_ERR_COST;
  state_cost += (distance / (ROAD_WIDTH / 2))*(distance / (ROAD_WIDTH / 2))*(distance / (ROAD_WIDTH / 2)) * TRACK_ERR_COST;
...
void trackInit(){
  ...
  Track* dev_mid_fcn,* host_in_fcn, * host_out_fcn;
...
void kernelmain(Control* output_cmd, Control* host_U, State* output_state, int st_freq) {
  ...
  genControlNew << <K, int(N_HRZ) >> > (time(NULL), dev_V, dev_U, V_RAND_SCALAR, W_RAND_SCALAR);  // Initialize random control perturbations to the nominal control U -> V
