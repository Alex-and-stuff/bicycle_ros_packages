...
#define W_RAND_SCALAR 0.5f //0.5 // 0.6
...
// #define OFF_ROAD_COST	  5.0f		//-100- // 300 Penalty for leaving the road
// #define COLLISION_COST  150.0f		//-500- //200 80.0f Penalty for colliding into an obstacle 100  140
// #define TRACK_ERR_COST	20.0f		//-100- //15.0f// Penalty for tracking error (parting from the middle) 3.0 3.0 20
// #define OBS_SIZE        1.0f

#define LAMBDA          350.0f //350
#define OFF_ROAD_COST	  200.0f		//-100- // 300 Penalty for leaving the road
#define COLLISION_COST  800.0f		//-500- //200 80.0f Penalty for colliding into an obstacle 100  140
#define TRACK_ERR_COST	5.0f		//-100- //15.0f// Penalty for tracking error (parting from the middle) 3.0 3.0 20
#define OBS_SIZE        1.0f
...
__device__ float distanceFromTrack(float* inner, float* nominal, Track* inner_fcn, Pos* turn_circle_intersec, State* state) {
  ...
  float OFF_ROAD_RENALTY = 0.0;
	...
	if(inner[0] < 0 && nominal[0] <= 0 && nominal[7] >= 0){ 
		return fabs(fabs(pointToLineDist(inner[0], inner_fcn[0])) - 0.5 * ROAD_WIDTH);  // respect to line: f
	}
	if (inner[1] > 0 && nominal[1] >= 0 && nominal[2] <= 0) {
		return fabs(fabs(pointToLineDist(inner[1], inner_fcn[1])) - 0.5 * ROAD_WIDTH);  // respect to line: g
	}
	if (inner[2] > 0 && nominal[3] <= 0 && nominal[4] >= 0) {
		return fabs(fabs(pointToLineDist(inner[2], inner_fcn[2])) - 0.5 * ROAD_WIDTH);  // respect to line: h
	}
	if (inner[3] < 0 && nominal[5] <= 0 && nominal[6] >= 0) {
		return fabs(fabs(pointToLineDist(inner[3], inner_fcn[3])) - 0.5 * ROAD_WIDTH);  // respect to line: i
	}
...
void kernelmain(Control* output_cmd, Control* host_U, State* output_state, int freq, float* cost, Obs* obs_pos, int obs_num){
  ...
  costFcn << <int(N_HRZ), K >> > (dev_state_costList, dev_stateList, dev_out_fcn, dev_in_fcn, dev_nominal_fcn, dev_obstacle, dev_turn_circle_intersec);
