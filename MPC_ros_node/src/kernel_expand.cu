...
#define W_RAND_SCALAR 0.5f
#define W_DIFF        5.0f//0.04f  //0.4
#define SCALE_MODEL   0.6f//0.61f //0.72f
#define RHO           0.05f//0.08f
#define DELTA_MAX     0.6f
#define DELTA_MIN     -0.6f

// good: 200 800 5 | 1 2 5 (W:1.1)
#define OFF_ROAD_COST	  8000.0f		// Penalty for leaving the road
#define COLLISION_COST  140.0f		// Penalty for colliding into an obstacle 100
#define TRACK_ERR_COST	3.0f		// Penalty for tracking error (parting from the middle) 3.0
...
__global__ void PertCost(float* costList, Control* rand, Control* u) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  costList[i] += LAMBDA * (u[threadIdx.x].vd * V_RAND_SCALAR * rand[i].vd + u[threadIdx.x].wd * W_RAND_SCALAR * rand[i].wd);
}
